...
def ImportAedatDataVersion3(aedat):
    ...
    if 'packetPointers' in info:
        ...
        packetTypes = np.ones(10000000, np.uint16)
        packetPointers = np.zeros(10000000, np.uint64)
        packetTimeStamps = np.zeros(10000000, np.uint64)
                    ...
                    if allDataTypes or 'polarity' in info['dataTypes']:
                        ...
                        if currentLength == 0:
                            ...
                            while eventNumber > currentLength - polarityNumEvents:
                                polarityValid     = np.append(polarityValid,     np.zeros(currentLength, dtype=bool ))
                                ...
                                polarityPolarity  = np.append(polarityPolarity,  np.zeros(currentLength,dtype=bool))
                                currentLength = len(polarityValid)
